...
db_path = os.path.join('/tmp', 'database_final_v1.db') # Nama baru biar fresh
...
class Pelanggaran(db.Model):
    ...
    jenis_pelanggaran = db.Column(db.String(100), nullable=True)
...
data_kelas = {
    ...
    for nama, pw in data_kelas.items():
        cek = Pelanggaran.query.filter_by(nama_siswa=nama).first()
        if not cek:
            db.session.add(Pelanggaran(nama_siswa=nama, password=pw, kelas='10'))
    db.session.commit()
...
def login_siswa():
    if request.method == 'POST':
        ...
        return "Login Gagal! Cek Nama (Gunakan Huruf Kapital) & Password."
...
def siswa():
    hasil = None
    if request.method == 'POST':
        nama_cari = request.form.get('nama')
        hasil = Pelanggaran.query.filter(Pelanggaran.nama_siswa.like(f'%{nama_cari}%')).all()
    return render_template('siswa.html', hasil=hasil)
